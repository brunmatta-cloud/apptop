...
const AppLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div data-mobile={isMobile} className="flex min-h-screen w-full max-w-full overflow-x-hidden bg-background">
      {!isMobile && (
        <>
          <aside className={`fixed inset-y-0 left-0 z-30 flex flex-col overflow-hidden border-r border-sidebar-border bg-sidebar transition-all duration-200 ${desktopMenuHidden ? 'w-0 border-r-0' : 'w-64'}`}>
          ...
          {desktopMenuHidden && (
            <button
              ...
              className="fixed left-4 top-1/2 z-40 flex -translate-y-1/2 items-center gap-2 rounded-r-2xl rounded-l-xl border border-border bg-card/90 px-2.5 py-3 text-muted-foreground shadow-lg backdrop-blur hover:bg-card hover:text-foreground"
      ...
      {isMobile && (
        <>
          <div className="fixed left-0 right-0 top-0 z-30 flex items-center justify-between border-b border-sidebar-border bg-sidebar/95 px-3 py-3 backdrop-blur-xl">
          ...
          <AnimatePresence>
            {mobileOpen && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed inset-0 z-40 bg-background/70 pt-16 backdrop-blur-xl"
              >
                <motion.div
                  initial={{ x: -280 }}
                  animate={{ x: 0 }}
