...
const LinhaDoTempoHeaderClock = React.memo(function LinhaDoTempoHeaderClock() {
  const { currentTime, formatTime } = useClock();
  return <span className="text-xl sm:text-2xl font-mono font-bold text-primary">{formatTime(currentTime)}</span>;
});
...
const TimelineItemsSection = React.memo(function TimelineItemsSection({
  safeMomentos,
  getMomentStatus,
  blockColors,
}: {
  safeMomentos: MomentoProgramacao[];
  getMomentStatus: (index: number) => string;
...
const LinhaDoTempo = () => {
  ...
  const totalMinutes = useMemo(() => safeMomentos.reduce((sum, momento) => sum + momento.duracao, 0), [safeMomentos]);
  const firstTime = safeMomentos[0]?.horarioInicio || '00:00';
  const lastEnd = safeMomentos.length > 0
    ? calcularHorarioTermino(safeMomentos[safeMomentos.length - 1].horarioInicio, safeMomentos[safeMomentos.length - 1].duracao)
    : '00:00';
  ...
  return (
    <div className="space-y-6">
      ...
      <TimelineItemsSection safeMomentos={safeMomentos} getMomentStatus={getMomentStatus} blockColors={blockColors} />
    </div>
  );
};
